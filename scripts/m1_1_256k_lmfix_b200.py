"""M1.1-256K — Decode SSD I/O byte trace at 256K context on B200.
...
CTX = 256000
log(f"M1.1-256K -- Decode SSD I/O at {CTX} context on B200")
...
for i in range(N):
    ...
    t_tok = time.time()
    with torch.no_grad():
        out = model(cur_ids, past_key_values=past_kv, use_cache=True, logits_to_keep=1)
    torch.cuda.synchronize()
...
result = {
    "measurement": f"M1.1-256K -- Decode SSD I/O at {CTX} context on B200",
    "date": "2026-05-27",
...
out_file = f"results/phase3/m1_1_decode_256k_lmfix_b200.json"
